...
package object opticsm
  ...
  implicit class ERefMSyntax[RA, RB, EA >: EB, EB, A, B](private val self: ZRefM[RA, RB, EA, EB, A, B]) {
    ...
    ): ZRefM[RA with RB, RB, EC, ED, Chunk[C], Chunk[D]] =
      self.foldAllM(
        identity,
        identity,
        identity,
   ...
   * Providing implicit syntax for accessing pieces of a value in a `ZRefM`.
   */
  implicit class ZRefMSyntax[RA, RB, EA, EB, A, B](private val self: ZRefM[RA, RB, EA, EB, A, B]) {

    /**
     * Accesses a term of a sum type.
     */
    final def accessCase[EC >: EA, ED >: EB, C, D](optic: Optic[B, Any, C, ED, EC, D, A]): ZRefM[RA, RB, EC, ED, C, D] =
      self.foldM(
        identity,
        identity,
  ...
  implicit class AtZRefMSyntax[RA, RB, EA >: EB, EB >: OpticFailure, A](
    private val self: ZRefM[RA, RB, EA, EB, Chunk[A], Chunk[A]]
  ) {

    /**
     * Accesses the specified index of a chunk.
     */
    def at(n: Int): ZRefM[RA with RB, RB, EA, EB, A, A] =
  ...
  implicit class ConsZRefMSyntax[RA, RB, EA, EB >: OpticFailure, A](
    private val self: ZRefM[RA, RB, EA, EB, List[A], List[A]]
  ) {

    /**
     * Accesses the `::` case of a `List`.
     */
    def cons: ZRefM[RA, RB, EA, EB, (A, List[A]), (A, List[A])] =
  ...
  implicit class FilterZRefMSyntax[RA, RB, EA >: EB, EB >: OpticFailure, A](
     ...
     */
    def filter(f: A => Boolean): ZRefM[RA with RB, RB, EA, EB, Chunk[A], Chunk[A]] =
  ...
  implicit class FirstZRefMSyntax[RA, RB, EA >: EB, EB, A, B, C](
    private val self: ZRefM[RA, RB, EA, EB, (C, B), (A, B)]
  ) {

    /**
     * Accesses the first element of a tuple.
     */
    def first: ZRefM[RA with RB, RB, EA, EB, C, A] =
  ...
  implicit class HeadZRefMSyntax[RA, RB, EA >: EB, EB >: OpticFailure, A](
     ...
     */
    def head: ZRefM[RA with RB, RB, EA, EB, A, A] =
  ...
  implicit class KeyZRefMSyntax[RA, RB, EA >: EB, EB >: OpticFailure, K, V](
    private val self: ZRefM[RA, RB, EA, EB, Map[K, V], Map[K, V]]
  ) {

    /**
     * Accesses the value at the specified key in a map.
     */
    def key(k: K): ZRefM[RA with RB, RB, EA, EB, V, V] =
  ...
  implicit class LeftZRefMSyntax[RA, RB, EA, EB >: OpticFailure, A, B, C](
    private val self: ZRefM[RA, RB, EA, EB, Either[C, B], Either[A, B]]
  ) {

    /**
     * Accesses the `Left` case of an `Either`.
     */
    def left: ZRefM[RA, RB, EA, EB, C, A] =
  ...
  implicit class NoneZRefMSyntax[RA, RB, EA, EB >: OpticFailure, A](
    private val self: ZRefM[RA, RB, EA, EB, Option[A], Option[A]]
  ) {

    /**
     * Accesses the `None` case of an `Option`.
     */
    def none: ZRefM[RA, RB, EA, EB, Unit, Unit] =
  ...
  implicit class RightZRefMSyntax[RA, RB, EA, EB >: OpticFailure, A, B, C](
    private val self: ZRefM[RA, RB, EA, EB, Either[A, C], Either[A, B]]
  ) {

    /**
     * Accesses the `Right` case of an `Either`.
     */
    def right: ZRefM[RA, RB, EA, EB, C, B] =
  ...
  implicit class SecondZRefMSyntax[RA, RB, EA >: EB, EB, A, B, C](
    private val self: ZRefM[RA, RB, EA, EB, (A, C), (A, B)]
  ) {

    /**
     * Accesses the second element of a tuple.
     */
    def second: ZRefM[RA with RB, RB, EA, EB, C, B] =
  ...
  implicit class SliceZRefMSyntax[RA, RB, EA >: EB, EB >: OpticFailure, A](
     ...
     */
    def slice(from: Int, until: Int): ZRefM[RA with RB, RB, EA, EB, Chunk[A], Chunk[A]] =
  ...
  implicit class SomeZRefMSyntax[RA, RB, EA, EB >: OpticFailure, A, B](
    private val self: ZRefM[RA, RB, EA, EB, Option[B], Option[A]]
  ) {

    /**
     * Accesses the `Some` case of an `Option`.
     */
    def some: ZRefM[RA, RB, EA, EB, B, A] =
  ...
  implicit class TailZRefMSyntax[RA, RB, EA >: EB, EB >: OpticFailure, A](
     ...
     */
    def tail: ZRefM[RA with RB, RB, EA, EB, List[A], List[A]] =
